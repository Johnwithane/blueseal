# Application Pack — Futurpreneur Canada (Core Startup)

> **Program:** Futurpreneur Canada — Core Startup. An **equity-free loan up to $75,000 total**
> ($25K Futurpreneur + $50K BDC co-loan), plus **up to 2 years of 1:1 mentorship.**
> **Non-dilutive** (it's a loan, not equity).
> **Fit:** Strong — **eligibility confirmed.** Purpose-built for pre-revenue founders.
>
> ✅ **Age gate cleared:** Johnny is **37** (within the 18–39 cap), so **Johnny is the named
> applicant/borrower.** Futurpreneur eligibility is per-applicant, so the loan goes in the eligible
> founder's name — James's age is irrelevant to it.
>
> *Official: [futurpreneur.ca/en/offering/core-startup](https://futurpreneur.ca/en/offering/core-startup/) ·
> [eligibility](https://futurpreneur.ca/en/eligibility/). Verified June 2026.*

---

## Terms (verified)

| Item | Detail |
| --- | --- |
| Loan amount | **Up to $75,000 total** = up to **$25,000** (Futurpreneur) + up to **$50,000** (BDC co-loan). *Earlier "$125K" figures are out of date.* |
| Structure | Equity-free **loan** (non-dilutive); 5-year term |
| Interest | Futurpreneur portion: **CIBC prime + 3%** (capped at 9% if CIBC prime > 6%). BDC portion: **BDC floating base rate + 1.65%** |
| Mentorship | **Up to 2 years** of hand-matched 1:1 mentorship (a real asset — fills the legal/finance/ops gap) |
| Age | **18–39 at application** (hard cap) — ✅ Johnny is 37 |
| Citizenship | Canadian citizen or permanent resident |
| Incorporation | Not required to *start*; **business registration needed before funds disburse** (so register/incorporate before the money lands). *Sole-prop vs must-incorporate not stated on the official page — confirm by phone.* |
| Required docs | **Complete business plan + 24-month cash flow** |

---

## What you need to submit

1. **Business plan** — assemble from the narrative bank (`00`): company description, problem/solution, market, competition, business model, team, go-to-market, use of funds.
2. **24-month cash-flow projection** — ✅ **drafted**: see [`07-futurpreneur-financials.md`](./07-futurpreneur-financials.md) + the working model [`futurpreneur-cashflow-24mo.csv`](./futurpreneur-cashflow-24mo.csv). Needs your accountant to confirm loan rates + the salary-vs-SR&ED call. (Reuse for New Ventures BC Round 3 financials.)
3. **Loan use & repayment narrative** — what the $75K does and how revenue repays it (covered in `07`).

---

## ✍️ DRAFT — answers to the core questions (in the founders' voice)

> Grounded in the live product; edit to fit Futurpreneur's exact form fields.

**Describe your business.**
> Blue Seal is a progressive web app that verifies skilled tradespeople and runs the whole job from quote to payment. We hand-check each tradesperson's trade certification and government ID before their profile goes live, then give clients and pros one place to search, quote, chat, schedule, invoice, pay, and review each other — with AI built into each step. It's live in BC's Okanagan with roughly 100 verified tradespeople across 134 trades. We earn through a capped 5% client service fee and a $29/month "Blue Seal Pro" subscription for tradespeople.

**What problem do you solve, and for whom?**
> Homeowners can't tell a genuinely qualified tradesperson from someone who just claims a trade — almost no one checks the certification or ID. And skilled tradespeople, excellent at their craft, run their business on texts and spreadsheets while competing on ad spend instead of quality. Blue Seal serves both: verified trust for homeowners, and modern tools to win and manage work for tradespeople — timely as BC faces a shortage of roughly 52,600 tradespeople this decade.

**Why are you the right team?**
> Two brothers with complementary skills. James is a Red Seal tradesperson with 20+ years in the trades who lived this problem building his own home — he leads product, sales, and marketing (and is a professional photographer). Johnny is an award-winning creative director who builds and maintains the platform and owns the brand. Trade credibility plus technical and brand execution — and Futurpreneur's mentorship would directly strengthen our finance and operations side.

**How will you use the loan?**
> Primarily local demand marketing in the Okanagan (acquiring clients onto an already-stocked, verified supply side), plus incorporation/legal setup, a security and QA pass, accounting setup, and app-store presence. The product is already built and live, so this is a go-to-market loan, not a build loan — it funds turning verified supply into paying transactions, which generates the revenue to repay it. *(Mirror the partner-brief allocation; right-size to the loan amount.)*

**How will you repay it?**
> Revenue comes from the capped service fee on card payments and Blue Seal Pro subscriptions ($29/mo, ~$300 contribution per pro per year at near-zero acquisition cost). As verified pros convert to Pro and client transactions grow, recurring subscription revenue covers repayment. *(Back with the 24-month cash flow.)*

---

## Action steps

- [x] **Age gate confirmed** — Johnny is 37 (≤39). ✓
- [x] **Named applicant: Johnny** (the eligible founder).
- [ ] Call/confirm with Futurpreneur whether a **sole prop qualifies** or incorporation is required before funds land *(plan to register/incorporate regardless — see `01`)*.
- [x] **24-month cash flow drafted** (`07` + CSV) — review with accountant; confirm loan rates + the salary/SR&ED call.
- [ ] Assemble the **business plan** from the narrative bank.
- [ ] Submit; expect to be matched with a **mentor** — lean on them for the finance/ops gap.

*Community Futures Central Okanagan* (`06`, up to $150K, no age cap) remains available as a **complement or larger alternative** — you're not limited to one loan path.
