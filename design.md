...
**Gating:** the AI assistant is a **Blue Seal Pro** feature (re-gated 2026-06-11 — see MONETIZATION.md). Every AI callable routes through `requireAiEntitlement` (functions/src/lib/entitlements.ts), which requires an active Pro subscription (trialing/active/past_due grace) or an unexpired founding comp — EXCEPT receipt OCR, which stays free for everyone. Admins bypass. Entitlement state lives on `users/{uid}.subscription`; `tradespeople/{uid}.isPro` is the public mirror. (The 2026-05 "open to all + 12% commission" interim model is gone; the old `hasActiveSubscription` boolean was replaced by the richer `subscription` object.)
