...
export interface PartyInfo {
  tradesperson: {
    ...
    // One-line profile headline (e.g. "Red Seal plumbing & gas across the
    // Okanagan"). Free for everyone — mirrors the tradesperson's public
    // profile so the quote/invoice letterhead carries the same branding.
    // Shown under the company name in the FROM panel.
    tagline?: string | null;
...
// the tab's lifetime. This is the current Blue Seal horizontal lockup
// (brandmark + wordmark, white) — the same logo the homepage + pitch deck
// use, rendered on the navy header band of every default (non-Pro) PDF.
let logoDataUrlCache: string | null = null;
async function loadBrandLockupDataUrl(): Promise<string | null> {
  if (logoDataUrlCache) return logoDataUrlCache;
  try {
    const res = await fetch("/branding/Horizontal/BlueSeal_Logo_Hor_White.png");
...
async function renderPdf(
  ...
  if (bannerDataUrl) {
    ...
    // Left side of the band. A Pro tradesperson's square logo takes the slot
    // with their company name as the wordmark; otherwise we render the current
    // Blue Seal horizontal lockup (brandmark + wordmark, white) — the same logo
    // the homepage + pitch deck use — as a single aspect-preserved image.
    ...
    if (usingTradieLogo) {
      ...
      try {
        doc.addImage(
          tradieLogoDataUrl,
          inferImageFormat(tradieLogoDataUrl),
          logoX,
          logoY,
          logoSize,
          logoSize,
      ...
      const wordmarkTitle = party.tradesperson.companyName?.trim() || party.tradesperson.name;
      doc.setFont("helvetica", "bold");
      doc.setFontSize(20);
      doc.setTextColor(...titleColor);
      ...
      doc.setFontSize(8.5);
      doc.setTextColor(...subtitleColor);
      // Tagline lives in the FROM panel (uniform across every header variant),
      // so the band keeps the role label here to avoid showing it twice.
      doc.text("Verified Tradesperson", textXAfterLogo, 58);
    } else if (brandLockupDataUrl) {
      ...
      try {
        ...
        doc.addImage(
          brandLockupDataUrl,
          inferImageFormat(brandLockupDataUrl),
          margin,
          (bandHeight - lockupH) / 2,
          lockupW,
          lockupH,
        );
      } catch {
        /* lockup add can fail on a malformed data URL — band still anchors the header */
      }
    } else {
      // No image available — keep the wordmark as a text fallback so the
      // header is never empty.
      doc.setFont("helvetica", "bold");
      doc.setFontSize(20);
      doc.setTextColor(...titleColor);
      doc.text("BlueSeal", margin, 44);
      doc.setFont("helvetica", "normal");
      doc.setFontSize(8.5);
      doc.setTextColor(...subtitleColor);
  ...
  // 4 contact lines (address, phone, email, GST/HST) without overflow. Grows
  // by one row when the tradesperson has a tagline (rendered under the company
  // name in the FROM panel); both panels share the height so they stay level.
  const tradieTagline = party.tradesperson.tagline?.trim() || null;
  const panelHeight = tradieTagline ? 128 : 112;
  ...
  const writePartyBlock = (
    ...
    // Tagline — the tradesperson's profile headline, italic in brand blue so it
    // reads as a strapline rather than another contact line. Clamped to one
    // line so it never overflows the fixed-height panel.
    if (tagline) {
      y += 12;
      doc.setFont("helvetica", "italic");
      doc.setFontSize(9);
      doc.setTextColor(...BRAND.blue);
      const oneLine = doc.splitTextToSize(tagline, colWidth - panelPadX * 2)[0] ?? tagline;
      doc.text(oneLine, x + panelPadX, y);
    }
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.setFontSize(8);
    doc.setTextColor(...BRAND.muted);
    // Middle dot, not an em dash: jsPDF's default Helvetica renders an em dash
    // as garbled spaced glyphs (same reason the totals use a plain hyphen).
    // periodcentered is in the standard Helvetica encoding and renders cleanly.
    doc.text(
