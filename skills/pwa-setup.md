...
The `VitePWA` config sets `registerType: "prompt"` (not `"autoUpdate"`) so a freshly-deployed service worker *waits* instead of force-reloading the tab. [`useAppUpdate.ts`](../src/composables/useAppUpdate.ts) owns *when* it applies — see the "stale assets" gotcha below for the full update story (visible banner + critical force-update lever).
...
- **Cache + stale assets (the "I don't see my deploy until I hard-refresh" bug).** A service worker only re-checks for a new `sw.js` on a fresh *navigation*. Desktop reloads tabs constantly so it's invisible there — but mobile (and installed PWAs) *resume* the app instead of reloading, so the check never fires and the user is stuck on the old build. **Auto-applying silently isn't enough on its own:** a "safe moment to reload" (app backgrounded or user idle) never arrives in an actively-used foreground browser tab, and when the tab *is* backgrounded mobile browsers freeze it before the off-screen reload can finish — so silent auto-apply lands for installed apps but not browser tabs. Blue Seal's full fix lives in [`src/composables/useAppUpdate.ts`](../src/composables/useAppUpdate.ts) + [`AppUpdatePrompt.vue`](../src/components/AppUpdatePrompt.vue):
  1. **Detect** two ways — vite-plugin-pwa's `needRefresh` (a waiting SW), *and* a poll of `/version.json` (a tiny network-only manifest that doesn't depend on the SW at all, so it works even when the SW is wedged). Re-checked on resume (`visibilitychange`/`focus`) + every 30 min.
  2. **Surface** a visible **Update** banner the user can tap — applying in the foreground by a real gesture sidesteps the frozen-tab problem entirely. Silent auto-apply at safe moments stays as a bonus for installed apps.
  3. **Force** critical fixes — `/version.json` carries a `critical` flag. Ship the fix with `CRITICAL_RELEASE=1 npm run deploy:prod` and every client on an older build gets a blocking "Update required" overlay (not just a nudge). `version.json` is emitted by a small plugin in [vite.config.ts](../vite.config.ts), the build id is baked in via `define` (`__APP_VERSION__`), and it's kept out of the Workbox precache (`globIgnores`).

  Wired in once at the app root ([App.vue](../src/App.vue)). NB: the cache *headers* must also be right — `index.html`, `sw.js`, **and `version.json`** need `Cache-Control: no-cache` ([firebase.json](../firebase.json)) or the browser never even fetches the new worker / manifest to compare it.
