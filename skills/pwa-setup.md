...
- **Cache + stale assets.** `registerType: "autoUpdate"` updates the SW in the background but the *current* tab keeps the old cached assets until reload. For most apps this is fine. If you need a "new version available, reload" toast, switch to `registerType: "prompt"` and use `useRegisterSW` from `virtual:pwa-register/vue`.
