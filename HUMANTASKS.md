...
- **What:**
  ...
  4. **Grant the CI deploy account access to the secret** — without this, any CI functions deploy that includes these two functions fails at "Loading and analyzing source code" with `403 secretmanager.secrets.get` (exactly the 2026-06-08/06-12 episode with the Google + Stripe secrets; deploy run 32510339390 on 2026-08-21 is this failure — hosting still shipped, and the functions were already live from a local deploy, so nothing is currently broken in prod). **Correction (2026-08-21): this probably does NOT need an Owner.** The earlier "`secretmanager.secrets.getIamPolicy` denied for both accounts" finding was a `gcloud` artifact, not a real permission gap: `gcloud`'s stored credential for `johnnyajansen@gmail.com` is actually a `marketing@wishboneltd.com` token, so *every* `gcloud --project=blueseal-762af` call returns PERMISSION_DENIED regardless of the account flag (see [docs/DISASTER_RECOVERY.md](./docs/DISASTER_RECOVERY.md)). Under ADC, which really is `johnnyajansen@gmail.com`, `getIamPolicy` on this secret succeeds and returns the existing `secretAccessor` binding. So the grant can be made locally once `gcloud auth login` is fixed, or right now via the REST API under ADC. Failing that, the console route still works: **Security → Secret Manager → `BUGS_GITHUB_TOKEN` → Permissions → Grant access** to `blueseal-ci@blueseal-762af.iam.gserviceaccount.com` with role **Secret Manager Admin** (resource-scoped on this one secret, same as the four existing secret grants).
...
### [ ] Re-authenticate `gcloud` for `johnnyajansen@gmail.com`

- **Why:** `gcloud`'s stored credential for `johnnyajansen@gmail.com` is actually a `marketing@wishboneltd.com` token. Proof: `gcloud storage buckets describe gs://blueseal-762af.firebasestorage.app --account=johnnyajansen@gmail.com` fails with *"marketing@wishboneltd.com does not have storage.buckets.get access"*, and `gcloud projects list` for that account returns Wishbone's projects, not Johnny's. Consequence: **every** `gcloud --project=blueseal-762af` command returns PERMISSION_DENIED, and that failure tells you nothing about the account's real access. It has already produced at least one wrong conclusion in this file (the `BUGS_GITHUB_TOKEN` "needs an Owner" note above).
- **What:** `gcloud auth login` and sign in as the real `johnnyajansen@gmail.com`. Consider `gcloud config configurations create blueseal` so the Wishbone config stops being the active one. Meanwhile: the Firebase CLI and ADC (`gcloud auth application-default print-access-token`) are both correctly authenticated as Johnny and are the working paths. See [docs/DISASTER_RECOVERY.md](./docs/DISASTER_RECOVERY.md).
- **Verify:** `gcloud firestore databases describe --database='(default)' --project=blueseal-762af` returns the database instead of PERMISSION_DENIED.
