...
- **What:**
  ...
  4. **Grant the CI deploy account access to the secret** — without this, any CI functions deploy that includes these two functions fails at "Loading and analyzing source code" with `403 secretmanager.secrets.get` (exactly the 2026-06-08/06-12 episode with the Google + Stripe secrets; deploy run 32510339390 on 2026-08-21 is this failure — hosting still shipped, and the functions were already live from a local deploy, so nothing is currently broken in prod). **Correction (2026-08-21): this does NOT need to wait on anyone. Johnny IS the Owner and can run it himself.** The earlier "`secretmanager.secrets.getIamPolicy` denied for both accounts" finding was a `gcloud` artifact, not a real permission gap: `gcloud`'s stored credential for `johnnyajansen@gmail.com` was a `marketing@wishboneltd.com` token, so *every* `gcloud --project=blueseal-762af` call returned PERMISSION_DENIED regardless of the account flag. That is now fixed (see the re-auth entry below), and `projects get-iam-policy` confirms `johnnyajansen@gmail.com` holds **`roles/owner`**. `get-iam-policy` on this secret now succeeds and shows only the compute service account holding `secretAccessor`, i.e. `blueseal-ci@` is genuinely missing. **✅ DONE 2026-08-21** — granted with:
     `gcloud secrets add-iam-policy-binding BUGS_GITHUB_TOKEN --member=serviceAccount:blueseal-ci@blueseal-762af.iam.gserviceaccount.com --role=roles/secretmanager.admin --project=blueseal-762af --account=johnnyajansen@gmail.com`
     Audited every other secret at the same time: `blueseal-ci@` now holds the grant on all five secrets the functions actually `defineSecret()` (`BUGS_GITHUB_TOKEN`, `GOOGLE_OAUTH_CLIENT_SECRET`, `GOOGLE_TOKEN_ENC_KEY`, `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`). The sixth secret in the project, `firestore-send-email-SMTP_PASSWORD`, belongs to a Firebase Extension that is not in `firebase.json`, so CI never reads it and it was deliberately left ungranted (least privilege). **Steps 1-3 above are still outstanding** — the secret still holds the `UNSET` placeholder until a real PAT is created. If it had been needed, the console route also still works: **Security → Secret Manager → `BUGS_GITHUB_TOKEN` → Permissions → Grant access** to `blueseal-ci@blueseal-762af.iam.gserviceaccount.com` with role **Secret Manager Admin** (resource-scoped on this one secret, same as the four existing secret grants).
...
### [x] Re-authenticate `gcloud` for `johnnyajansen@gmail.com` — ✅ DONE 2026-08-21

- **Why it mattered:** `gcloud`'s stored credential for `johnnyajansen@gmail.com` was actually a `marketing@wishboneltd.com` token, so **every** `gcloud --project=blueseal-762af` command returned PERMISSION_DENIED regardless of `--account`, and that failure told you nothing about real access. It had already produced a wrong conclusion in this file (the `BUGS_GITHUB_TOKEN` "needs an Owner" note above).
- **Resolved:** Johnny re-ran `gcloud auth login`. Verified: `gcloud projects list` now returns `blueseal-762af`, `firestore databases describe` returns the database, and `projects get-iam-policy` confirms `johnnyajansen@gmail.com` holds **`roles/owner`**.
- **Still worth knowing:** the machine has two gcloud accounts and the *active configuration* is `[wishbone]` with project `bettertour`, so always pass `--project=blueseal-762af --account=johnnyajansen@gmail.com` explicitly or you will act on the wrong project. Documented in [docs/DISASTER_RECOVERY.md](./docs/DISASTER_RECOVERY.md).
- **Lesson:** treat any older "we don't have permission for that" note on this project as suspect until re-tested.

### [x] Export the Firebase Auth users — ✅ FIRST EXPORT DONE 2026-08-21, but it needs re-running

- **Why:** Firestore backups and PITR went live 2026-08-21, but **neither covers Firebase Authentication**. Losing the auth store means every client, tradesperson, PM and rep re-registers, and all their uid-keyed data (jobs, quotes, invoices, reviews, Stripe Connect links) orphans with no way to reattach it.
- **Done:** exported 31 accounts (25 with password hashes) to `C:\Users\Johnny\blueseal-backups\`, **outside the repo**, next to a `README-RESTORE.md` holding the import command and the SCRYPT hash parameters. Those parameters are **not** in the export file, and without them `auth:import` cannot restore a single password, which is the usual way this backup turns out to be worthless at the worst moment.
- **⚠️ Johnny, two things only you can do:**
  1. **Move that folder somewhere encrypted.** It holds every user's email, 25 password hashes, and the project's password-hash signer key. Together they are enough to recreate every account on the platform. Password manager file vault or an encrypted drive. Not OneDrive, not email.
  2. **Re-export periodically.** It is a point-in-time dump, not a schedule, so it is worth roughly the day it was taken. Command is in the README.
- **Restore caveats (documented in [docs/DISASTER_RECOVERY.md](./docs/DISASTER_RECOVERY.md)):** `customAttributes` carries the role claims that drive every Firestore rule, so verify roles after any import. And never replay an export blind: `scheduledHardDelete` erases auth accounts on PIPEDA request, so a wholesale import resurrects accounts we were legally required to delete.
