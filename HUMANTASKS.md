...
- **What:**
  ...
  4. **Grant the CI deploy account access to the secret** — without this, any CI functions deploy that includes these two functions fails at "Loading and analyzing source code" with `403 secretmanager.secrets.get` (exactly the 2026-06-08/06-12 episode with the Google + Stripe secrets; deploy run 32510339390 on 2026-08-21 is this failure — hosting still shipped, and the functions were already live from a local deploy, so nothing is currently broken in prod). **Correction (2026-08-21): this does NOT need to wait on anyone. Johnny IS the Owner and can run it himself.** The earlier "`secretmanager.secrets.getIamPolicy` denied for both accounts" finding was a `gcloud` artifact, not a real permission gap: `gcloud`'s stored credential for `johnnyajansen@gmail.com` was a `marketing@wishboneltd.com` token, so *every* `gcloud --project=blueseal-762af` call returned PERMISSION_DENIED regardless of the account flag. That is now fixed (see the re-auth entry below), and `projects get-iam-policy` confirms `johnnyajansen@gmail.com` holds **`roles/owner`**. `get-iam-policy` on this secret now succeeds and shows only the compute service account holding `secretAccessor`, i.e. `blueseal-ci@` is genuinely missing. **The whole fix is one command:**
     ...
     The console route also still works: **Security → Secret Manager → `BUGS_GITHUB_TOKEN` → Permissions → Grant access** to `blueseal-ci@blueseal-762af.iam.gserviceaccount.com` with role **Secret Manager Admin** (resource-scoped on this one secret, same as the four existing secret grants).
...
### [ ] Export the Firebase Auth users periodically (the one backup gap)
